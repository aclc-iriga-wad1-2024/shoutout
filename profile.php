...
else if(isset($_SESSION['shoutout_username']) && isset($_SESSION['shoutout_password'])) {
    // stored in session
...
if($tab == 'settings' && (!isset($_SESSION['shoutout_username']) || !isset($_SESSION['shoutout_password']) || $profile['username'] !== $_SESSION['shoutout_username'] || $profile['password'] !== $_SESSION['shoutout_password'])) {
    $tab = 'received';
}
...
if(isset($_POST['shout-out']) && isset($_SESSION['shoutout_username']) && isset($_SESSION['shoutout_password']) && $profile['id'] != $user['id'])
{
...
if(isset($_POST['delete-shout-out']) && isset($_SESSION['shoutout_username']) && isset($_SESSION['shoutout_password']))
{
...
if(isset($_POST['update-account']))
{
    ...
    if($error['account'] == '') {
        ...
        if($stmt->affected_rows > 0) {
            ...
            $_SESSION['shoutout_username'] = $username;
            if(isset($_COOKIE['shoutout_username']))
                setcookie('shoutout_username', $username, time() + (7 * 24 * 60 * 60));
            header('location: profile.php?id=' . $user['id'] . '&tab=settings');
        }
    }
}
...
if(isset($_POST['update-password']))
{
    ...
    if($error['password'] == '') {
        ...
        // update password in session and remembered data (if available)
        $_SESSION['shoutout_password'] = $hashed_password;
        if(isset($_COOKIE['shoutout_password']))
            setcookie('shoutout_password', $hashed_password, time() + (7 * 24 * 60 * 60));
    }
}
?>
...
<main class="container pt-3">
    ...
    <div class="bg-white border-top-0 border-b" style="border: 1px solid #dee2e6">
        <!-- shout-out form -->
        <?php if(isset($_SESSION['shoutout_username']) && isset($_SESSION['shoutout_password']) && $profile['id'] != $user['id'] && $tab == 'received') { ?>
            <div class="p-3 pb-0">
                                ...
                                <div class="d-flex justify-content-between align-items-center">
                                    ...
                                    <h6 class="mb-1 fw-bold">
                                        <a href="profile.php?id=<?= $shout_out['user_id'] ?>" class="text-decoration-none d-flex align-items-end gap-1">
                                            ...
                                            <div>
                                                <?php
                                                if($shout_out['user_id'] == $user['id'] && isset($_SESSION['shoutout_username']) && isset($_SESSION['shoutout_password'])) {
                                                    echo "(me) ";
                                                }
                                                ?>
                                        ...
                                        <i>
                                            ...
                                            <b>
                                                ...
                                                if($shout_out['user_mode'] == 'selected') {
                                                    ...
                                                    if($shout_out['selected_user'] == $user['id'] && isset($_SESSION['shoutout_username']) && isset($_SESSION['shoutout_password'])) {
                                                        echo "(me) ";
                                                    }
