...
if(isset($_POST['sign-in']))
{
    ...
    // if a user is found, verify the entered password against the stored hashed password
        ...
        if(password_verify($password, $row['password'])) {
            // check if the user is already email-verified
            if($row['email_verified_at'] == '') {
                ...
                if($remember) {
                    ...
                    setcookie('shoutout_username', $row['username'], $cookie_expiration);
                    setcookie('shoutout_password', $row['password'], $cookie_expiration);
                }
                // otherwise, remove any remembered data if still available
                else if(isset($_COOKIE['shoutout_username']) || isset($_COOKIE['shoutout_password'])) {
                ...
                <div class="card-body">
                    <form method="POST" action="sign-in.php">
                        ...
                        <div class="mt-2 d-flex justify-content-end">
                            ...
                            <small class="px-2 opacity-50">|</small>
                            <small><a href="recover.php" class="text-decoration-none">Forgot password</a></small>
                        </div>
                    </form>
                </div>
            </div>
